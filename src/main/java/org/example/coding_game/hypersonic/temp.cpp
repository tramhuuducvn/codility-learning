...
template <typename T>
vector<vector<T>> vectors(T a, size_t h, size_t w) { return vector<vector<T>>(h, vector<T>(w, a)); }
...
namespace primitive
{
    ...
    entity_t entity_cast(player_t const &a)
    {
        entity_t b;
        b.player = a;
        return b;
    ...
    entity_t entity_cast(bomb_t const &a)
    {
        entity_t b;
        b.bomb = a;
        return b;
    ...
    entity_t entity_cast(item_t const &a)
    {
        entity_t b;
        b.item = a;
        return b;
    ...
    istream &operator>>(istream &in, turn_t &a)
    {
        ...
        repeat(y, a.config.height)
        {
            repeat(x, a.config.width)
            {
                ...
                a.field[y][x] =
                    c == '.' ? cell_t::empty : c == 'X' ? cell_t::wall
                                                        : cell_t(c - '0');
            }
        }
        int n;
    ...
    item_kind_t open_item_box(cell_t a)
    {
        switch (a)
        {
        case cell_t::box_extra_range:
            return item_kind_t::extra_range;
        case cell_t::box_extra_bomb:
            return item_kind_t::extra_bomb;
        default:
            assert(false);
        }
    }
...
struct exploded_time_info_t
{
    int time;
    bool owner[player_number];
};

exploded_time_info_t default_explosion_info()
{
    exploded_time_info_t a = {inf};
    return a;
}
...
bool is_valid_commands(turn_t const &turn, map<player_id_t, command_t> const &commands)
{
    ...
    for (player_t &ent : players)
    {
        if (commands.count(ent.id))
        {
            ...
            {
                if (not is_on_field(command.y, command.x, turn.config.height, turn.config.width))
                    return false;
                if (abs(command.y - ent.y) + abs(command.x - ent.x) >= 2)
                    return false;
                if (turn.field[command.y][command.x] != cell_t::empty)
                    return false;
                if (bombs.count(point(command)))
                    return false;
            }
...
{
    if (not is_valid_commands(cur, commands))
        return nullptr;
    info = {};
    ...
    for (entity_t ent : cur.entities)
    {
        if (exptime[ent.y][ent.x].time - 1 == 0)
        {
            ...
            case entity_type_t::player:
                ...
                if (ent.player.id == cur.config.self_id)
                    return nullptr;
                break;
            case entity_type_t::bomb:
    ...
    for (auto &it : items)
    {
        ...
        if (player_exists.count(point(ent)))
            continue;
        nxt->entities.push_back(entity_cast(ent));
    }
    return nxt;
...
bool is_survivable(player_id_t self_id, turn_t const &turn, vector<vector<exploded_time_info_t>> const &exptime)
{
    ...
    repeat(t, bomb_time)
    {
        ...
        repeat(y, h) repeat(x, w)
        {
            if (not prv[y][x])
                continue;
            if (exptime[y][x].time - 1 == t)
                continue; // TODO: 同じ座標を異なる複数の時刻に爆風が通るのを漏らしてる
            repeat(i, 5)
            {
                int ny = y + dy[i];
                int nx = x + dx[i];
                if (not is_on_field(ny, nx, h, w))
                    continue;
                if (turn.field[ny][nx] == cell_t::wall)
                    continue;
                if (is_box(turn.field[ny][nx]) and exptime[ny][nx].time - 1 >= t)
                    continue;
                if (point(ny, nx) != point(y, x) and bombs.count(point(ny, nx)) and exptime[ny][nx].time - 1 >= t)
                    continue;
                cur[ny][nx] = true;
                exists = true;
            }
                ...
                for (auto const &pho : beam)
                {
                    repeat(i, 5) repeat(j, 2)
                    {
                        map<player_id_t, command_t> commands;
                        {
                            ...
                            if (age == 0 and forbidden.count(command))
                                continue;
                        ...
                        if (used.count(npho->signature))
                            continue;
                        used.insert(npho->signature);
                        if (not is_survivable(self.id, npho->turn, npho->exptime))
                            continue;
